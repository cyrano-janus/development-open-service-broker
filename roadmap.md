...
| **OSB Checker (standalone)** | [`osb-checker`](./osb-checker) | Manuelles Voll-Audit-Tool gegen beliebige Broker (21 Checks, config.yaml-gesteuert). Das CI-Conformance-Gate (Phase 4.2) läuft mit dem konsolidierten `cmd/osb-checker` im Broker-Repo — **24 Checks, funktional aufeinander abgestimmt** |
